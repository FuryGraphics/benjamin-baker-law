...
export default function PracticeAreas({ isFullPage = false, setRoute }: PracticeAreasProps) {
  ...
  if (isFullPage) {
    ...
    return (
      <main className="bg-[#FAF8F3] min-h-screen pt-32 pb-24 px-5 sm:px-6 lg:px-8 relative" id="practice-areas-page">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            ...
            <div className="lg:col-span-4 space-y-3" id="practice-areas-sidebar">
              <span className="font-sans text-[10px] tracking-[4px] text-[#5F5B53] uppercase block mb-3 font-semibold px-4">
              ...
              {PRACTICE_AREAS.map((area) => (
                <button
                  ...
                  className={`w-full text-left p-5 transition-all duration-300 flex items-center justify-between border ${
                    activeTab === area.id
                      ? 'bg-[#FFFFFF] border-[#C9A84C] text-[#9A7A28] shadow-lg shadow-[#000]/40'
                      : 'bg-transparent border-[rgba(201,168,76,0.12)] text-[#1F1D1A]/70 hover:bg-[#FFFFFF]/40 hover:text-[#1F1D1A]'
                  }`}
                  id={`tab-btn-${area.id}`}
                >
                  <div className="flex items-center space-x-4">
                    {renderIcon(area.iconName, `w-5 h-5 ${activeTab === area.id ? 'text-[#C9A84C]' : 'text-[#5F5B53]'}`)}
                    <span className="font-serif text-lg tracking-wider font-medium">{area.title}</span>
                  </div>
                  <ChevronRight className={`w-4 h-4 transition-transform duration-300 ${activeTab === area.id ? 'translate-x-1 text-[#C9A84C]' : 'text-[#5F5B53]/50'}`} />
                </button>
              ))}

              <div className="bg-[#FFFFFF]/60 border border-[rgba(201,168,76,0.12)] p-6 mt-8 space-y-4 text-center rounded-sm">
                ...
                <button 
                  ...
                  className="w-full bg-[#C9A84C] text-[#0C0C0C] font-sans text-xs font-bold tracking-widest uppercase py-3 hover:bg-[#9A7A28] active:bg-[#C9A84C] transition-colors cursor-pointer"
            ...
            <div className="lg:col-span-8 bg-[#FFFFFF] border border-[rgba(201,168,76,0.18)] p-8 lg:p-12 shadow-2xl relative" id="practice-depth-display-board">
              ...
              <div className="flex items-center space-x-4 border-b border-black/[0.08] pb-6 mb-8">
                <div className="border border-[#C9A84C]/40 p-3 bg-black/[0.03] rounded-full">
                  {renderIcon(activeArea.iconName, "w-8 h-8 text-[#C9A84C]")}
                </div>
                <div>
                  <h2 className="font-serif text-2xl sm:text-3xl text-[#9A7A28] font-semibold tracking-wide">
                    {activeArea.title} Representation
                  </h2>
                  <p className="font-sans text-[11px] tracking-widest text-[#5F5B53] uppercase mt-1">
                    Benjamin A. Baker P.C. · Sherman, TX
                  </p>
                </div>
              </div>

              <p className="font-serif italic text-xl text-[#1F1D1A]/90 leading-relaxed mb-8 border-l-2 border-[#C9A84C] pl-6">
                "{activeArea.summary}"
              </p>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5" id="specialties-subgrid">
                    {activeArea.details.map((detail, count) => (
                      <div key={count} className="flex items-start space-x-3 bg-black/[0.02] p-3.5 border border-black/[0.07] rounded-sm hover:border-[#C9A84C]/25 transition-colors">
                        <CheckCircle2 className="w-4 h-4 text-[#C9A84C] shrink-0 mt-0.5" />
                        <span className="font-sans text-xs text-[#1F1D1A]/90 leading-normal font-medium">{detail}</span>
                      </div>
                    ))}
                  </div>
                </div>

                <div className="border-t border-black/[0.08] pt-6 mt-8">
                  <h4 className="font-serif text-[#9A7A28] text-lg font-medium mb-3">Courtroom Philosophy</h4>
                  <p className="font-sans text-xs text-[#5F5B53] leading-relaxed">
                ...
                <div className="bg-[#FAF8F3] p-6 border border-[rgba(201,168,76,0.15)] flex flex-col sm:flex-row items-center justify-between gap-4 mt-8">
                  <div className="text-center sm:text-left">
                    <span className="font-sans text-[10px] tracking-[4px] text-[#5F5B53] uppercase block">
                ...
                <div className="bg-[#FAF8F3] p-6 border border-[rgba(201,168,76,0.18)] flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
                  <div className="text-center sm:text-left">
                    <span className="font-sans text-[10px] tracking-[4px] text-[#5F5B53] uppercase block">
                      SCHEDULE A TEST CASE REVIEW
                    </span>
                    <span className="font-serif text-base text-[#1F1D1A] block mt-1">
                      Let us evaluate your legal objectives today.
                    </span>
                  </div>
                  <button
                    onClick={() => setRoute('contact')}
                    className="bg-[#C9A84C] hover:bg-[#9A7A28] text-[#0C0C0C] font-sans text-xs font-bold tracking-widest uppercase py-3 px-6 transition-colors shadow-md shadow-black/[0.08]"
  ...
  return (
    <section 
      className="bg-[#FAF8F3] py-24 px-5 sm:px-6 lg:px-8 border-b border-[rgba(201,168,76,0.1)] relative"
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-left mb-16 max-w-2xl">
          ...
          <h2 id="practice-areas-title" className="font-serif text-3xl sm:text-4xl lg:text-5xl text-[#1F1D1A] font-medium leading-tight relative pb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8" id="practice-cards-grid">
          {PRACTICE_AREAS.map((area, index) => (
            <motion.div
              ...
              className="bg-[#FFFFFF] border border-[rgba(201,168,76,0.15)] gold-glow p-8 flex flex-col justify-between group rounded-sm"
              id={`practice-card-${area.id}`}
            >
              <div>
                {/* Gold Outline Icon (32px) */}
                <div className="mb-6 inline-block p-3 border border-[rgba(201,168,76,0.22)] bg-black/[0.02] rounded-sm">
                  {renderIcon(area.iconName, "w-8 h-8 text-[#C9A84C]")}
                </div>
                
                {/* Practice Name */}
                <h3 className="font-serif text-[#9A7A28] text-xl sm:text-2xl font-semibold tracking-wide mb-3">
                  {area.title}
                </h3>
                
                {/* 2-sentence Summary */}
                <p className="font-sans text-xs sm:text-sm text-[#5F5B53] leading-relaxed mb-6 group-hover:text-[#1F1D1A]/80 transition-colors">
                  {area.summary}
                </p>
              </div>
              ...
              <button
                ...
                className="inline-flex items-center space-x-2 text-[#C9A84C] hover:text-[#9A7A28] text-xs font-semibold font-sans uppercase tracking-widest pt-2 cursor-pointer focus:outline-none"
